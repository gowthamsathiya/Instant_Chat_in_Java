...
public class goChatClient{
	...
	public static void initConfiguration() {
		disconnectButton.setVisible(false); //disable disconnect button
		sendButton.setVisible(false); //disable send button
		connectButton.setVisible(true); //enable connect button
		connectButton.setFocusable(true); //set focus on connect button
		chattingAsLabel.setText("Connected as "+nameTextField.getText()); //change the chattingAsLabel text
	}
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(nameTextField.getText().equals(username)){
				...
				try {
					...
					COut.println("REC#$ > "+goChatClient.username+" > "+recepient); // send REC#$ - Connect to particular recipient
					COut.flush();
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private ActionListener getconnectedListener = new ActionListener(){
		...
		public void actionPerformed(ActionEvent e) {
			if(!nameTextField.getText().equals("")){
				...
				layout.show(panel,"chathomepanel"); //change the view of frame to chat home panel card
				chattingAsLabel.setText("Chatting as "+username);
				frame.setTitle("goChat - "+username);
			}
			else{
				JOptionPane.showMessageDialog(null, "Enter some valid name"); //error message on invalid name entered
			}
		}
	...
	public static void configurationAfterConnect(){
		disconnectButton.setVisible(true); //set disconnect button to visible
		sendButton.setVisible(true);//set send button to visible
		sendButton.setFocusable(true);//set focus on send button
		connectButton.setVisible(false);//set connect button to invisible 
		connectButton.setFocusable(false);//remove focus from connect button 
	}
	...
	protected void connectToServer() {
		try{
			...
			final int port = 80; //port number of server
			clientsocket = new Socket("localhost",port); //establish socket connection with server
			chatclient = new goChatClientThread(clientsocket);
			
			PrintWriter out = new PrintWriter(clientsocket.getOutputStream());
			String vcheck = "0";
			if(visibleCheckBox.isSelected())
				vcheck = "1"; //set 1 when visible radio button is checked
			out.println(vcheck+nameTextField.getText());
			out.flush();
			
			Thread t = new Thread(chatclient); //assign separate thread to listen for message from server
			t.start(); //start the thread
		}
		catch(Exception e){
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Oops!. Server went down"); //show when server went down
			System.exit(0);
		}
	}
	...
	protected void sendChatMessage(String chatMessage) throws UnsupportedEncodingException {
		...
		chatclient.sendMessage(header+" > "+chatMessage); //append header message with chat message
		chatMessageTextField.setText(""); //remove chat message from message text field to enter next message
		chatMessageTextField.requestFocus(); //give the focus to text field to enter next message
	}
	...
	protected void disconnect() throws IOException {
		...
		chatclient.disconnect(); //call disconnect method in goChatClientThread
		initConfiguration(); // change frame configuration to initial one
	}
	...
	protected void logOff(){
		try{
			chatclient.logOff(); // call logOff method in goChatClientThread
			JOptionPane.showMessageDialog(null, "Logging off. Bye");
			System.exit(0);//exit the client GUI
		}
		catch(Exception e){
			e.printStackTrace();
		}
	...
	/**
	 * Scroll pane listener to scroll down to latest message
	 */
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 478, 555);//allocated position to the element in the frame window
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.getContentPane().setLayout(null);
		
		panel = new JPanel();
		panel.setBounds(0, 0, 462, 517);//allocated position to the element in the frame window
		...
		lblGochat.setBounds(149, 107, 140, 96);//allocated position to the element in the frame window
		...
		lblToGetConnected.setBounds(236, 196, 126, 19);//allocated position to the element in the frame window
		panel_1.add(lblToGetConnected);
		
		JLabel lblName = new JLabel("Name :");
		lblName.setBounds(55, 350, 46, 14);//allocated position to the element in the frame window
		panel_1.add(lblName);
		
		nameTextField = new JTextField();
		nameTextField.setBounds(130, 347, 232, 20);//allocated position to the element in the frame window
		panel_1.add(nameTextField);
		nameTextField.setColumns(10);
		
		visibleCheckBox = new JCheckBox("Visible to others");
		visibleCheckBox.setBounds(261, 381, 156, 23);//allocated position to the element in the frame window
		panel_1.add(visibleCheckBox);
		
		getConnectedButton = new JButton("getConnected");
		getConnectedButton.setBounds(160, 447, 129, 23);//allocated position to the element in the frame window
		...
		chattingAsLabel.setBounds(10, 11, 223, 39);//allocated position to the element in the frame window
		...
		lblVisibleUsers.setBounds(334, 10, 99, 40);//allocated position to the element in the frame window
		...
		visibleUserScrollPane.setBounds(299, 58, 153, 349);//allocated position to the element in the frame window
		...
		connectToLabel.setBounds(299, 418, 125, 20);//allocated position to the element in the frame window
		panel_2.add(connectToLabel);
		
		recepientNameTextField = new JTextField();
		recepientNameTextField.setBounds(299, 449, 114, 20);//allocated position to the element in the frame window
		panel_2.add(recepientNameTextField);
		recepientNameTextField.setColumns(10);
		
		connectButton = new JButton("~");
		connectButton.setBounds(411, 450, 41, 19);//allocated position to the element in the frame window
		panel_2.add(connectButton);
		
		chatMessageTextField = new JTextField();
		chatMessageTextField.setBounds(10, 431, 217, 54);//allocated position to the element in the frame window
		panel_2.add(chatMessageTextField);
		chatMessageTextField.setColumns(10);
		
		sendButton = new JButton(">");
		sendButton.setBounds(222, 431, 59, 55);//allocated position to the element in the frame window
		...
		conversationScrollPane.setBounds(10, 58, 269, 349);//allocated position to the element in the frame window
		...
		logOffButton.setBounds(222, 21, 88, 23);//allocated position to the element in the frame window
		panel_2.add(logOffButton);
		
		disconnectButton = new JButton("Disconnect");
		disconnectButton.setBounds(309, 483, 124, 23);//allocated position to the element in the frame window
